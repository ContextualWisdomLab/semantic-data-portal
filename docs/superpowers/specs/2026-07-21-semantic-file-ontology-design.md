...
- 파일 내용은 로컬에서 추출하며, 사용자가 승인한 최소 원문 조각만 OpenAI Responses API로 전송한다.
...
- 공급자 자격증명과 OpenAI API key는 그래프·문서·로그·GitHub에 저장하지 않는다.
...
6. OpenAI Responses API의 strict JSON Schema 출력으로 의미 후보와 짧은 근거 인용을 받는다.
...
10. 파일 node의 embedding text는 제목과 제안된 개념 label만 사용한다. 원문과 근거 인용문은 저장하지 않는다.

OpenAI API key는 extractor에 credential registry lookup으로 주입한다. 애플리케이션 코드가 `os.getenv()`로 key를 읽지 않는다. HTTP transport는 Python 표준 라이브러리를 사용하고 요청에 `store: false`와 pinned model id를 포함한다. CI에서는 HTTP 호출을 fake transport로 대체한다.
...
- storage credential과 OpenAI key가 없으면 fail closed 한다.
...
- OpenAI 요청은 strict schema, `store: false`, 최소 조각만 포함한다.
