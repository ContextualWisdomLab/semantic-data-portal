...
**10차(2026-09-16 10:07Z)까지 관측했습니다.** 열 번 모두 같은 모양입니다 — 몇 시간 대기한 뒤 스스로 풀리고, 시작된 뒤에는 정상 속도로 돌고, 결과는 green입니다. 대기 시간은 4시간 21분 / 6시간 01분 / 4시간 08분 / 5시간 20분 / 3시간 16분 / 3시간 26분 / 3시간 39분 / 3시간 16분 / 3시간 31분 / 5시간 04분으로 **일정하지 않습니다.** 따라서 "약 N시간이면 풀린다"는 식의 예측은 세우지 마십시오 — 이 문서도 한 번 그렇게 읽었다가 2차에서 틀렸습니다.

**표본이 늘수록 범위는 좁아지지 않고 넓어졌습니다.** 네 번까지는 4시간 08분~6시간 01분이었는데, 5차가 3시간 16분으로 그 하한 아래로 내려가 지금은 3시간 16분~6시간 01분입니다. 즉 관측을 더 해도 수렴하는 것이 아니라 양쪽으로 벌어지고 있으므로, **여기서 배운 어떤 임계값도 근거로 쓸 수 없습니다.**

6차 이후로는 경계가 더 움직이지 않았고, 여덟 번을 모아도 범위는 3시간 16분~6시간 01분 그대로입니다.
...
지금 말할 수 있는 것은 방향이 아니라 **하한이 반복된다**는 것뿐입니다. 최근 다섯 번이 3시간 16분 / 3시간 26분 / 3시간 39분 / 3시간 16분 / 3시간 31분으로 약 23분 폭 안에 모여 있고, 특히 3시간 16분이 두 번 정확히 재현되었습니다. 상한(6시간 01분)은 아직 한 번뿐입니다. 그래도 **이것으로 다음 대기를 예측하지는 마십시오** — 이 절은 같은 실수를 이미 두 번 했습니다.
...
따라서 위 표의 아홉 건은 **"다음 push 전에 러너를 받은 실행"만 모인 편향 표본**입니다. 대기가 길수록 다음 push에 잘리기 쉬우므로 편향의 방향은 분명합니다 — **짧은 쪽으로 치우칩니다.** 3시간 16분~6시간 01분이라는 범위, 특히 하한 부근에 다섯 건이 모였다는 관찰도 그만큼 할인해서 읽어야 합니다. 실제 분포의 꼬리는 여기 적힌 것보다 길 수 있습니다.

측정을 깨끗하게 하려면 큐에 실행이 떠 있는 동안 push하지 않아야 하지만, 이 문서는 취소를 피하려고 다른 작업을 미루지는 않았습니다. **편향이 있다는 사실을 적어 두는 쪽을 택합니다.**

**편향 방향은 한 번 확인되었습니다 (10차).** `35057397730`은 커밋할 것이 없어서 큐에 떠 있는 동안 한 번도 push하지 않은 첫 실행입니다. 결과는 **5시간 04분** — 직전 다섯 번이 모여 있던 3시간 16분~3시간 39분 구간보다 한참 위입니다. push를 멈추자 측정값이 길어졌다는 뜻이고, 하한 부근의 군집이 적어도 부분적으로는 push 주기가 만든 것이라는 위 설명과 맞아떨어집니다.
...
진단은 여전히 하지 않습니다. 다만 "그날의 단발성 기록"이 아니라 **2026-09-14 06:54Z부터 2026-09-16 09:58Z까지 이틀 넘게 이어진 조건**으로 읽으십시오. 다시 관측되면 이 표에 행을 더하십시오.
...
위 네 건의 측정값은 [해당 lane에 보고했습니다](https://github.com/ContextualWisdomLab/.github/pull/1150#issuecomment-5676434919). 보고에 담긴 요지는 측정 그 자체보다 **판정 기준에 대한 것**입니다. 그 lane의 RED 정의는 "러너 배정·checkout 신원·step이 없는 materialized job"인데, 위 네 건은 대기 중에 정확히 그렇게 보였다가 스스로 러너를 받아 SUCCESS로 끝났습니다. 즉 **어느 한 시점의 관측만으로는 RED와 "길지만 회복할 대기"를 구분할 수 없습니다.** 대기 시간으로도 구분되지 않습니다 — 3시간 16분에서 6시간 01분 사이로 흩어져 임계값을 세울 수 없기 때문입니다. 판정은 종료 결과를 보거나 그 외의 판별자를 써야 합니다.
