...
| Lineage DAG reconstruction / weekly report | LineageWeave (해당 저장소 issue/PR은 `owner/repo#num`으로 인용) | 산발 레코드에서 lineage thread를 재구성하는 것은 LineageWeave 소관입니다. 포털은 결과를 소비하고 provenance pointer만 저장하며, 어떤 KG write 경로도 호출하지 않습니다. LineageWeave 쪽 작업은 포털 블로커가 아닙니다. **정정(2026-09-07):** 이전 판은 이 줄에서 LineageWeave를 맨 `#74`로 인용했으나, 맨 `#74`는 이 저장소의 issue #74([Product Gap] framework-neutral data management evidence, `#75`가 구현)로 해석됩니다. 조직 규약대로 저장소 밖 참조는 `owner/repo#num` 또는 전체 URL로만 씁니다. |
