"""Language-neutral structural digest profile for admitted JSON values."""
...
def _encode_value(
    ...
    *,
    ...
    if isinstance(value, float):
        ...
        return f"f{struct.pack('>d', value).hex()};".encode("ascii")
    ...
    if type(value) is list:
        ...
        return f"a{len(value)}[".encode("ascii") + encoded_items + b"]"
    ...
    if type(value) is dict:
        ...
        try:
            ...
            active_containers.remove(identity)
        return f"o{len(value)}{{".encode("ascii") + encoded_members + b"}"
...
def encode_structural_json(value: object, field_name: str) -> bytes:
    """Encode JSON data with the versioned CWL structural grammar.

    The grammar preserves scalar types, signed 64-bit integers, IEEE-754
    binary64 values, strict UTF-8 strings, array order, and object-key order by
    UTF-8 bytes. Any grammar change requires a new digest profile identifier.
    """
...
def structural_sha256(value: object, field_name: str) -> str:
    ...
    return f"sha256:{sha256(encode_structural_json(value, field_name)).hexdigest()}"
