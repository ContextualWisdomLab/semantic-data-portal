...
def test_receipt_rejects_tampered_top_level_identity(
    ...
    """Top-level receipt identity cannot diverge from nested evidence."""
...
def test_receipt_rejects_projection_and_metadata_disagreement() -> None:
    """Release, authority, provenance, and omission metadata stay aligned."""
...
def test_receipt_rejects_tenant_source_or_time_tampering() -> None:
    """Projection scope, candidate identity, and event time remain coherent."""

    tenant_payload = _receipt_payload()
    tenant_payload["tenant_id"] = "tenant_other"
    with pytest.raises(
        ValidationError,
        match="projection_id does not match receipt scope",
    ):
    ...
    with pytest.raises(
        ValidationError,
        match="source_instance_id does not match projection",
    ):
        OpenMetadataAdmissionReceipt.model_validate(source_payload)

    time_payload = _receipt_payload()
    time_payload["observed_at"] = "2026-09-04T00:05:00Z"
    with pytest.raises(
        ValidationError,
...
def test_receipt_detaches_from_source_payload_mutation() -> None:
    """Changing caller-owned source data cannot rewrite existing evidence."""

    source_table = table_payload()
    receipt = preview_openmetadata_table_admission(
        tenant_id="tenant_acme",
        source_instance_id="metadata_primary",
        source_release="2.0.1",
    ...
    serialized_before = receipt.model_dump_json()

    source_table["displayName"] = "Changed after receipt"
    source_table["columns"] = []

    assert receipt.model_dump_json() == serialized_before
