...
from sdp.openmetadata import OpenMetadataContractError, normalize_openmetadata_table_snapshot
...
def _reference(reference_id: str, entity_type: str, name: str) -> dict[str, object]:
...
def _table_payload() -> dict[str, object]:
    ...
    return {
        ...
        "columns": [
            {
                ...
                "tags": [{"tagFQN": "Identifier.Primary", "source": "Classification"}],
            },
            {
                "name": "customer",
                "dataType": "STRUCT",
                "ordinalPosition": 2,
                "fullyQualifiedName": "warehouse.sales.public.orders.customer",
                "children": [
                    {
                        "name": "email",
                        "dataType": "VARCHAR",
        ...
        "databaseSchema": _reference("66666666-6666-4666-8666-666666666666", "databaseSchema", "warehouse.sales.public"),
        ...
        "tags": [{"tagFQN": "Tier.Tier1", "source": "Classification"}],
        "domains": [_reference("99999999-9999-4999-8999-999999999999", "domain", "sales")],
        "dataProducts": [_reference("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", "dataProduct", "customer_360")],
        "dataContract": _reference("bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", "dataContract", "orders_contract"),
        "profile": {
        ...
        "joins": {"directTableJoins": [{"fullyQualifiedName": "private.customer", "joinCount": 1}]},
    }
...
def _lineage_payload() -> dict[str, object]:
    ...
    upstream = _reference(_UPSTREAM_ID, "table", "warehouse.raw.orders")
    downstream = _reference(_DOWNSTREAM_ID, "dashboard", "sales.order_dashboard")
    primary = _reference(_TABLE_ID, "table", "warehouse.sales.public.orders")
    return {
        "entity": primary,
...
def test_normalizer_preserves_identity_semantics_and_safe_lineage() -> None:
    ...
    assert result.projection_id == f"urn:cwl:tenant_acme:sdp:openmetadata_table:{_TABLE_ID}"
    ...
    assert result.fully_qualified_name == "warehouse.sales.public.orders"
    ...
    assert upstream.column_mappings[0].from_columns == ["warehouse.raw.orders.order_id"]
    assert upstream.column_mappings[0].to_column == "warehouse.sales.public.orders.order_id"
...
def test_normalizer_rejects_lineage_for_another_primary_entity() -> None:
    ...
    lineage = _lineage_payload()
    lineage["entity"] = _reference(_UPSTREAM_ID, "table", "warehouse.raw.orders")

    with pytest.raises(OpenMetadataContractError, match="lineage entity does not match table id"):
...
def test_normalizer_rejects_lineage_edges_with_unknown_endpoints() -> None:
    """Every lineage endpoint must resolve to the primary entity or a declared node."""

    lineage = _lineage_payload()
    lineage["upstreamEdges"][0]["fromEntity"] = "cccccccc-cccc-4ccc-8ccc-cccccccccccc"

    with pytest.raises(OpenMetadataContractError, match="unknown lineage endpoint"):
        normalize_openmetadata_table_snapshot(
            tenant_id="tenant_acme",
            source_release="2.0.1",
            table=_table_payload(),
...
def test_normalizer_rejects_duplicate_column_paths() -> None:
    ...
    with pytest.raises(OpenMetadataContractError, match="duplicate column path: order_id"):
        normalize_openmetadata_table_snapshot(
            tenant_id="tenant_acme",
            source_release="2.0.1",
            table=table,
...
def test_normalizer_rejects_excessive_column_nesting() -> None:
    """Hostile recursive schemas are bounded before they can exhaust the process."""

    table = _table_payload()
    nested = {"name": "level_00", "dataType": "STRUCT", "children": []}
    table["columns"] = [nested]
    cursor = nested
    for index in range(1, 18):
        child = {"name": f"level_{index:02d}", "dataType": "STRUCT", "children": []}
        cursor["children"] = [child]
        cursor = child

    with pytest.raises(OpenMetadataContractError, match="column nesting exceeds 16"):
        normalize_openmetadata_table_snapshot(
            tenant_id="tenant_acme",
            source_release="2.0.1",
            table=table,
...
def test_openmetadata_normalization_endpoint_returns_typed_projection() -> None:
    """The HTTP surface exposes the same deterministic anti-corruption contract."""
    ...
    assert body["source_schema_uri"] == "https://open-metadata.org/schema/entity/data/table.json"
    assert body["lineage_schema_uri"] == "https://open-metadata.org/schema/type/entityLineage.json"
    assert body["columns"][2]["column_path"] == "customer.email"


def test_openmetadata_endpoint_rejects_pre_2_x_payload_contracts() -> None:
    """The first adapter slice does not silently reinterpret pre-2.x payloads."""
...
def test_openmetadata_endpoint_returns_bounded_contract_error() -> None:
    """Malformed external references produce a stable 400 without echoing payload data."""
