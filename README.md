...
OIDC token verification은 `SDP_OIDC_ISSUER`, `SDP_OIDC_AUDIENCE`, `SDP_OIDC_JWKS_URL`, `SDP_OIDC_GROUP_ROLE_MAP`를 사용하며, 서명 검증 후 Keyverse의 `org`/`role` claim을 tenant 및 제한된 SDP RBAC role로 매핑합니다. `preferred_username`/`email`/`sub`와 `org`/`tenant_id`는 비어 있지 않은 문자열이어야 하며, 숫자·배열·공백 값은 fail-closed로 거부합니다. `workspace`는 현재 workspace-scoped resource model이 없어 인증 claim shape로만 허용하며, tenant나 권한으로 사용하지 않습니다.
